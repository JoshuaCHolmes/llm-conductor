...
#[command(
    name = "llm-conductor",
    version,
    about = "Intelligent LLM orchestration"
)]
struct Cli {
...
async fn run_chat(resume: bool) -> Result<()> {
    ...
    if provider_config_manager.is_enabled("ollama") {
        match OllamaInstaller::check_installation().await {
            ...
            InstallStatus::NotInstalled => {
                ...
                eprintln!(
                    "{}",
                    "⚠️  Ollama not found but is enabled in config".bright_yellow()
                );
                eprintln!(
                    "Continuing without Ollama. To install, run: {}",
                    "llm-conductor setup".bright_white()
                );
            }
            _ => {}
        }
    ...
    if resume {
        ...
        if total > 0 {
            ...
            print!(
                "{} ",
                "Select session number (or press Enter to start fresh):".bright_cyan()
            );
            ...
            if let Some(usage) = usage_tracker.get_usage(&provider_id) {
                ...
                match &usage.limit_type {
                    ...
                    LimitType::RequestBased {
                        ...
                        println!(
                            " ({}/{} requests, resets {:?})",
                            formatted_remaining, max_requests, reset_period
                        );
                    }
                    ...
                    LimitType::CostBased {
                        ...
                        println!(
                            " ({}/{:.2}, resets {:?})",
                            formatted_remaining, max_cost, reset_period
                        );
                    }
                }
            } else {
...
async fn handle_config_command(cmd: ConfigCommands) -> Result<()> {
    ...
    match cmd {
        ...
        ConfigCommands::AddOutlier {
            ...
            println!(
                "{}",
                "⚠ Automated Outlier cookie extraction not yet implemented".yellow()
            );
            println!();
            println!("Please extract cookies manually:");
            println!(
                "1. Open {} in {}",
                "https://playground.outlier.ai".cyan(),
                browser
            );
            ...
            println!("Then run:");
            println!(
                "  llm-conductor config add-key outlier_cookie '_jwt=...; _session=...; _csrf=...'"
            );
